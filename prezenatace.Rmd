...
Implementace [Simple Features](https://cs.wikipedia.org/wiki/Simple_Features) standardu vektorových dat v R.

### objekty třídy `sf`
- rozšiřují `data.frame` = kombinují datové sloupce a "navíc" speciální sloupec *geometry* s prostorovými atributy
- platí pro ně stejné funkce a techniky jako pro `data.frame` - například `dplyr::inner_join()` a `dplyr::filter()`
...
## ✔ podporuje iteraci (a automatizaci)
