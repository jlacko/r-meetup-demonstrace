...
```{r tmap, eval = T, echo = F, cache = T, message = F, out.width = '100%', fig.asp = 0.55, dpi = 150}
...
```{r rczechia, eval = T, echo = T, cache = T, out.width = '100%', fig.asp = 0.55, dpi = 150}
...
## **Parametr** dojezdová vzdálenost ("okolí" z předchozího bodu)

---
# Relativní výsledky volby
Volební zisk v *% platných hlasů* po obcích a částech - tohle bývá v novinách...
```{r map-relative, eval = T, echo = F, cache = T, out.width = '100%', fig.asp = 0.67, dpi = 150}
...
Volební zisk *v počtu platných hlasů* po obcích a částech - protože na demonstraci přijdou lidé, ne procenta

```{r map-absolute, eval = T, echo = F, cache = T, out.width = '100%', fig.asp = 0.67, dpi = 150}

mapAbsolute <- tm_shape(podklad) + tm_fill(col = "zeman", title = "Volební zisk", n = 5) +
  tm_shape(kraje) + tm_borders("grey80") +
  tm_shape(republika) + tm_borders("grey35") +
  tm_style_white(frame = F, 
                 legend.format = list(text.separator =  "-",
...
st_agr(podklad) <- "constant" # vše jsou konstanty
...
```{r selmostroj, eval = T, echo = T, cache = T, message = F, warning = F, out.width = '100%', fig.asp = 0.67, dpi = 150}
selmostroj <- function(vzdalenost) {
  podklad$suma_volicu <- NA # uklidit
  ...
  for (i in 1:nrow(podklad)) { # pro všechny řádky podkladu
    ...
    podklad$suma_volicu[i] <- sum(isect$zeman) 
      # uložit součet hlasů v bufferu do podkladu
  }
  
 stred <- podklad[which.max(podklad$suma_volicu), ] # tato obec je optimální!
...
```{r spad-2, eval = T, echo = T, cache = T, message = F, warning = F, out.width = '100%', fig.asp = 0.67, dpi = 150}
selmostroj(2 * 1000) #2 kilometry
```
---
# Spádovost 10 kilometrů
```{r spad-10, eval = T, echo = T, cache = T, message = F, warning = F, out.width = '100%', fig.asp = 0.67, dpi = 150}
selmostroj(10 * 1000) #10 kilometrů
```
---
# Spádovost 30 kilometrů
```{r spad-30, eval = T, echo = T, cache = T, message = F, warning = F, out.width = '100%', fig.asp = 0.67, dpi = 150}
selmostroj(30 * 1000) # 30 kilometrů
```
---
# Spádovost 50 kilometrů
```{r spad-50, eval = T, echo = T, cache = T, message = F, warning = F, out.width = '100%', fig.asp = 0.67, dpi = 150}
selmostroj(50 * 1000) #50 kilometrů
```
---
# Spádovost 150 kilometrů
```{r spad-150, eval = T, echo = T, cache = T, message = F, warning = F, out.width = '100%', fig.asp = 0.67, dpi = 150}
